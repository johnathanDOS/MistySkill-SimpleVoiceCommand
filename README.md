...
3. Annotate the training phrases to highlight the parameter, or the word(s) with the relevant information for this request. The parameter for each of our ChangeLED training phrases is the color, so we annotate each color word (i.e. "blue") as a color parameter. This allows Dialogflow to recognize the color words as values that speakers will provide when our skill runs on Misty. ![Annotating training phrases](./img/annotate-training-phrases.gif) <br> From the [Dialogflow documentation](https://cloud.google.com/dialogflow/docs/intents-actions-parameters):
